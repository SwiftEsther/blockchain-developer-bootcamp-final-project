...
contract("NFT", async (accounts) => {
  ...
  it("burn: should increased burned by 2", async () => {
    const startingMinted = await instance.minted.call();
    ...
    await instance.burn([1, 2], { from: buyer });

    const endingBurned = await instance.burned.call();
    assert.equal(toBN(endingBurned).toString(), toBN(2).toString());
  });
});
